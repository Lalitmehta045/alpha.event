...
import { update_ADMIN } from "@/services/operations/adminOperations/allUsers";
...
export default function ViewAdminDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await update_ADMIN(user._id, token, updatedData);
      ...
      onOpenChange(false);
      fetchAdmins(); // Optional: Refresh table after update
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-11/12 md:max-w-md">
        <DialogHeader className="flex justify-between items-center">
          <DialogTitle className="text-xl">Admin Details</DialogTitle>
        </DialogHeader>
