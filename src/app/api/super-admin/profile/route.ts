...
import { ensureSuperAdmin } from "@/lib/adminGuard";
// ✅ Must return logged-in admin ID

// ✅ GET Logged-in Admin Profile
export async function GET(req: NextRequest) {
  try {
    const adminId = await ensureSuperAdmin(req); // Extract admin ID from token/session
    await connectDB();
...
export async function PUT(req: NextRequest) {
  try {
    const adminId = await ensureSuperAdmin(req);
    await connectDB();
