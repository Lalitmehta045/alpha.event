...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[upload-image] Received: ${file.name} | ${file.type} | ${(buffer.length / 1024).toFixed(0)} KB`
    );
    ...
    // Handles: HEIC conversion, resize, and JPEG compression.
    // This is a safety net — the client should already have compressed,
    // but this protects against HEIC files that bypass client conversion
    // and large files from non-standard clients.
    try {
      let pipeline = sharp(buffer);
      const metadata = await pipeline.metadata();

      console.log(
        `[upload-image] Metadata: ${metadata.width}×${metadata.height} | format: ${metadata.format}`
      );
      ...
      if (needsResize || needsConvert) {
        ...
        console.log(
          `[upload-image] Processed: ${(buffer.length / 1024).toFixed(0)} KB (resized: ${needsResize}, converted: ${needsConvert})`
        );
      }
    } catch (sharpError) {
      // If sharp fails (unsupported format etc.), continue with original buffer
      console.error("[upload-image] Sharp processing failed, using original:", sharpError);
    }
